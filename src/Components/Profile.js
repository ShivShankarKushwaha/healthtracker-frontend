...
function Profile(props) {
  ...
  let [user,setuser] =useState({name:"",email:""});
    useEffect(()=>
    {
        ...
        {
            let data = await fetch('/userdata',{credentials:'include'});
            if(data.status==200)
            {
            ...
            else
            {
              ...
              profilecontainer.innerHTML =`<h1>Sorry No Data found</h1>`
            }
        }
        getuserdata();
    },[]);
  ...
  return (
    <div className='w-full lg:border-2 h-96' id='profilecontainer'>
